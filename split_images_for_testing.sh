...
EXTENDED_TIF="$WORKDIR/$(md5sum "$INPUT_GTIFF" | cut -f 1 -d ' ').tif"
gdal_translate -q -srcwin 0 0 $X_SIZE_EXTEND $Y_SIZE_EXTEND "$INPUT_GTIFF" $EXTENDED_TIF
IFS='
'
#for X in $(seq 1 $N_PATCH_X); do for Y in $(seq 1 $N_PATCH_Y); do subset_patch_image "$WORKDIR/$(basename "$INPUT_GTIFF")" $X $Y; done; done
parallel subset_patch_image $EXTENDED_TIF {} {} ::: $(seq 1 $N_PATCH_X) ::: $(seq 1 $N_PATCH_Y)
